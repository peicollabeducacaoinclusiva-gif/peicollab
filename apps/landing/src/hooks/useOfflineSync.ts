...
import { offlineDB, syncUtils } from '@/lib/offlineDatabase';
  ...
  const syncData = useCallback(async () => {
    ...
    try {
      ...
      for (const pei of unsyncedRecords.peis) {
        ...
        if (!error) {
          await offlineDB.markAsSynced('peis', pei.id);
        } else {
      ...
      for (const goal of unsyncedRecords.pei_goals) {
        ...
        if (!error) {
          await offlineDB.markAsSynced('pei_goals', goal.id);
        } else {
      ...
      for (const barrier of unsyncedRecords.pei_barriers) {
        ...
        if (!error) {
          await offlineDB.markAsSynced('pei_barriers', barrier.id);
        } else {
      ...
      await syncUtils.setLastSyncTime(Date.now());

      // Recarregar status
      const newLastSyncTime = await syncUtils.getLastSyncTime();
      const newUnsyncedRecords = await offlineDB.getUnsyncedRecords();
