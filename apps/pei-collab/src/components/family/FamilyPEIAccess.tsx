  ...
  const validateToken = async () => {
    try {
      ...
      const { data: tokenData, error: tokenError } = await supabase
        ...
        .select('id, student_id, pei_id, expires_at, used, max_uses, current_uses')
        .eq('token_hash', token)
        .maybeSingle();
      ...
      const { data: pei, error: peiError } = await supabase
        .from('peis')
        .select('id, status, diagnosis_data, planning_data, evaluation_data, family_approved_at, family_approved_by, created_at, updated_at, school_id')
        .eq('id', tokenData.pei_id)
        .maybeSingle();

      if (peiError) throw peiError;
      if (!pei) throw new Error('PEI associado ao token não encontrado.');
      ...
      if (!student) throw new Error('Estudante associado ao token não encontrado.');

      let schoolName = 'Escola não identificada';
      const schoolId = student.school_id || pei.school_id;
      ...
      if (schoolId) {
        ...
        if (schoolError) {
          console.warn('Não foi possível carregar a escola:', schoolError.message);
        } else if (school?.school_name) {
