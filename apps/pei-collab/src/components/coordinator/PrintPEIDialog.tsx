...
const PrintPEIDialog = ({ peiId, open, onClose }: PrintPEIDialogProps) => {
  ...
  const [includeEvaluations, setIncludeEvaluations] = useState(false);
          ...
          <div className="space-y-4">
            <div className="flex items-center space-x-2 p-4 bg-muted/50 rounded-lg">
              <Checkbox 
                ...
                onCheckedChange={(checked) => setIncludeEvaluations(checked as boolean)}
              />
              <Label 
                htmlFor="include-evaluations"
                className="text-sm font-medium cursor-pointer"
              >
                Incluir pareceres e avaliações no documento impresso
              </Label>
            </div>
            <Button onClick={handlePrint} className="w-full">
              <Printer className="mr-2 h-4 w-4" />
              Imprimir PEI {includeEvaluations && "(com avaliações)"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
          ...
          <div className="print-only-content">
            ...
            <div className="flex items-start gap-4 mb-4 pb-3 border-b-2 border-black">
              ...
              <div className="flex-1 text-center">
                <h2 className="text-lg font-bold mb-1 leading-tight uppercase">{pei.tenant?.network_name || "Nome da Rede"}</h2>
                <p className="text-sm font-semibold text-gray-800 mb-2 leading-tight">Secretaria de Educação - Setor Educação Inclusiva</p>
                <p className="text-sm font-bold text-gray-900 leading-tight">{pei.school?.school_name || "Nome da Escola"}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              ...
              <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[8pt]">
                <p><strong>Nome:</strong> {pei.student?.name}</p>
                {pei.student?.date_of_birth && (
                  <p><strong>Nascimento:</strong> {format(new Date(pei.student.date_of_birth), "dd/MM/yyyy", { locale: ptBR })}</p>
                )}
            ...
            {includeEvaluations && (reviews.length > 0 || evaluations.length > 0) && (
              <div className="mt-4 pt-4 border-t-2 border-black">
                <h2 className="text-lg font-bold mb-3 text-center bg-gray-100 py-2">
                ...
                {reviews.length > 0 && (
                  <div className="mb-4">
                    <h3 className="text-sm font-bold mb-2 pb-0.5 border-b border-gray-500">
                      Pareceres do Coordenador
                    </h3>
                    <div className="space-y-3">
                      {reviews.map((review, idx) => (
                        <div key={review.id} className="bg-gray-50 p-2 border-l-4 border-blue-500">
                          <div className="flex justify-between items-start mb-1">
                            <p className="text-[8pt] font-bold">
                              Parecer #{idx + 1} - {format(new Date(review.review_date), "dd/MM/yyyy", { locale: ptBR })}
                            </p>
                            {review.meeting && (
                              <p className="text-[7pt] text-gray-600">
                                Reunião: {review.meeting.title}
                              </p>
                            )}
                          </div>
                          <p className="text-[7pt] text-gray-600 mb-1">
                            Revisor: {review.reviewer?.full_name || "N/A"}
                          </p>
                          <p className="text-[8pt] whitespace-pre-wrap leading-tight">
                            {review.notes}
                          </p>
                        </div>
                      ))}
                  ...
                  <div className="mb-4">
                    ...
                    <div className="space-y-3">
                      {evaluations.map((evaluation) => (
                        <div key={evaluation.id} className="bg-purple-50 p-2 border-l-4 border-purple-500">
                          <div className="flex justify-between items-start mb-1">
                            <p className="text-[8pt] font-bold">
                              {evaluation.cycle_name} - Ciclo {evaluation.cycle_number}
                            </p>
                            <p className="text-[7pt] text-gray-600">
                          ...
                          {evaluation.evaluated_at && (
                            <p className="text-[7pt] text-gray-600 mb-1">
                              Avaliado em: {format(new Date(evaluation.evaluated_at), "dd/MM/yyyy", { locale: ptBR })}
                            </p>
                          )}
                          <div className="grid grid-cols-3 gap-2 text-[7pt] text-center mb-2 mt-1">
                            <div className="bg-green-100 p-1 rounded">
                              <p className="font-bold text-green-700">✓ {evaluation.goals_achieved?.length || 0}</p>
                              <p className="text-[6pt]">Alcançadas</p>
                            </div>
                            <div className="bg-yellow-100 p-1 rounded">
                              <p className="font-bold text-yellow-700">◐ {evaluation.goals_partially_achieved?.length || 0}</p>
                              <p className="text-[6pt]">Parciais</p>
                            </div>
                            <div className="bg-red-100 p-1 rounded">
                              <p className="font-bold text-red-700">✗ {evaluation.goals_not_achieved?.length || 0}</p>
                              <p className="text-[6pt]">Não Alcançadas</p>
                            </div>
                          </div>
                          {evaluation.coordinator_recommendations && (
                            <div className="mt-1">
                              <p className="text-[7.5pt] font-semibold">Recomendações do Coordenador:</p>
                              <p className="text-[7.5pt] whitespace-pre-wrap leading-tight text-gray-700">
                                {evaluation.coordinator_recommendations}
                              </p>
                            </div>
                          )}
