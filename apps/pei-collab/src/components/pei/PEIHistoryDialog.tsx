...
interface PEIHistoryRecord {
  ...
  evaluation_data: ReferralsData; // evaluation_data agora é ReferralsData
  status: string;
}
  ...
  const loadHistory = async () => {
    ...
    try {
      ...
      const formattedHistory = historyData.map((record: any) => ({
        ...
        // Garantir que os dados aninhados estejam no formato correto
        diagnosis_data: record.diagnosis_data || { interests: "", specialNeeds: "", barriers: [], history: "", cid10: "", description: "" },
        planning_data: record.planning_data || { goals: [] },
        evaluation_data: record.evaluation_data || { referrals: [], observations: "" },
      }));
      ...
      <DialogContent className="max-w-4xl max-h-[80vh]">
        ...
        {versionData ? (
          <PEIVersionCompare
            ...
            onClose={() => setSelectedVersions([null, null])}
          />
        ) : (
