...
const SchoolManagerDashboard = ({ profile }: SchoolManagerDashboardProps) => {
  ...
  const loadTenants = async () => {
    ...
    const tenantList = data?.map((t: any) => ({
      id: t.school_id,
      name: t.school?.school_name || "Escola",
    })) || [];
    ...
    try {
      ...
      const [{ data: studentsData, error: studentsError }, { data: peisData, error: peisError }, { data: usersData, error: usersError }] =
        await Promise.all([
          ...
          supabase.from("profiles").select("*, school:schools!profiles_school_id_fkey(school_name)").eq("school_id", activeTenant),
        ]);
  ...
  const handleCSVExport = () => {
    ...
    downloadTextFile(fileName, csvWithBOM, 'text/csv;charset=utf-8;');
