  ...
  const loadSchoolStats = async () => {
    try {
      ...
      const { data: peisData, error: peisError } = await supabase
        .from('peis')
        .select('id, status, created_at, updated_at, assigned_teacher_id, student_id')
        .eq('school_id', userSchoolId);
      ...
      const peisByStatus = {
        completed: peisData?.filter(p => p.status === 'approved').length || 0,
        pending: peisData?.filter(p => p.status === 'pending').length || 0,
        approved: peisData?.filter(p => p.status === 'approved').length || 0,
      };

      // Calcular tempo médio de criação (em dias)
      const creationTimes = peisData
        ?.map(pei => {
          const created = new Date(pei.created_at);
          const updated = pei.updated_at ? new Date(pei.updated_at) : new Date();
      ...
      // Calcular professores com PEIs
      const teachersWithPEIsSet = new Set(
        peisData?.map(p => p.assigned_teacher_id).filter(Boolean) || []
      );

      // Calcular total de PEIs por professor
      const teacherPEICounts = peisData?.reduce((acc, pei) => {
        if (pei.assigned_teacher_id) {
          acc[pei.assigned_teacher_id] = (acc[pei.assigned_teacher_id] || 0) + 1;
        }
        return acc;
      ...
      const processedStats: SchoolStats = {
        ...
        peisCompleted: peisByStatus.completed,
        peisPending: peisByStatus.pending,
        peisApproved: peisByStatus.approved,
        averageCreationTime: Math.round(averageCreationTime * 10) / 10,
        teachersWithPEIs: teachersWithPEIsSet.size
      };
      ...
      const processedTeachers: TeacherData[] = teachersData?.map(teacher => ({
        ...
        totalPEIs: teacherPEICounts[teacher.id] || 0,
        completedPEIs: peisData?.filter(p => 
          p.assigned_teacher_id === teacher.id && p.status === 'approved'
        ).length || 0,
        pendingPEIs: peisData?.filter(p => 
          p.assigned_teacher_id === teacher.id && p.status === 'pending'
        ).length || 0,
