...
const SpecialistDashboard = ({ profile }: SpecialistDashboardProps) => {
  ...
  const loadSpecialistData = async () => {
    try {
      ...
      let studentsMap = new Map<string, { id: string; name: string; date_of_birth: string | null }>()
      ...
      const peisWithCounts: PEIWithStudent[] = await Promise.all(
        (peisData ?? []).map(async (pei) => {
          ...
          const studentInfo = pei.student_id ? studentsMap.get(pei.student_id) : undefined
          ...
          return {
            ...
            student: studentInfo ?? {
              id: pei.student_id ?? "unknown",
              name: studentInfo?.name ?? "Aluno não identificado",
