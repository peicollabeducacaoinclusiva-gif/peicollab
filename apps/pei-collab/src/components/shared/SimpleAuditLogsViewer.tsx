...
export function SimpleAuditLogsViewer({ 
  ...
  const loadLogs = async () => {
    try {
      ...
      // Se não tiver tenantId, tentar obter do perfil do usuário atual
      ...
      if (!resolvedTenantId) {
        try {
          ...
          if (user) {
            ...
            resolvedTenantId = profile?.tenant_id || profile?.school_id || null;
          }
        } catch (err) {
          console.warn('Erro ao obter tenantId do perfil:', err);
        }
      }

      // Se ainda não tiver tenantId, mostrar mensagem
      if (!resolvedTenantId) {
      ...
      const transformedData = (data || []).map((item: any) => ({
        ...
        school_id: item.tenant_id, // Mapear tenant_id para school_id para compatibilidade
  ...
  const getActionBadgeVariant = (action: string) => {
    ...
    switch (actionLower) {
      ...
      case 'anonymize': return 'destructive';
  ...
  const getActionLabel = (action: string) => {
    ...
    switch (actionLower) {
      ...
      case 'read': return 'Visualizado';
      case 'export': return 'Exportado';
      case 'anonymize': return 'Anonimizado';
