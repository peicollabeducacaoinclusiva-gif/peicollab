...
const getPrimaryRole = (profile: Profile): UserRole => {
  ...
  const role = profile.role || profile.user_roles?.[0]?.role || 'teacher'
...
const Dashboard = () => {
    ...
    try {
      ...
      if (profileData) {
        ...
        if (profileData.tenant_id) {
          ...
          networkName = profileData.tenant?.network_name
        }
        
        // Se o usuário tem school_id, buscar o tenant da escola
        if (profileData.school_id) {
          finalSchoolId = profileData.school_id
          schoolName = profileData.school?.school_name
          finalTenantId = profileData.school?.tenant_id || profileData.tenant_id
        }
      }
  ...
  const renderDashboard = () => {
    ...
    switch (getPrimaryRole(profile)) {
      case "superadmin":
        return <SuperadminDashboard profile={profileForDashboards} />
      case "education_secretary":
        return <EducationSecretaryDashboard />
      case "school_director":
        return <SchoolDirectorDashboard />
      case "coordinator":
        return <CoordinatorDashboard profile={profileForDashboards} />
      case "teacher":
        return <TeacherDashboard profile={profileForDashboards} />
      case "family":
        return <FamilyDashboard profile={profileForDashboards} />
      case "school_manager":
        return <SchoolManagerDashboard profile={profileForDashboards} />
      case "aee_teacher":
        return <AEETeacherDashboard profile={profileForDashboards} />
      case "specialist":
        return <SpecialistDashboard profile={profileForDashboards} />
          ...
          <div className="grid grid-cols-3 items-center gap-2 sm:gap-4">
            ...
            <div className="flex flex-col items-center justify-center text-center">
              <div className="flex items-center gap-1 sm:gap-2">
                ...
                <h1 className="font-bold text-base sm:text-xl bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
                  PEI Collab
                </h1>
              </div>
            ...
            <div className="flex items-center justify-end gap-1 sm:gap-2">
              ...
              <AppSwitcher currentApp="pei-collab" />
              <MobileNavigation userRole={userRole || "teacher"} />
              {(userRole || "teacher") !== "superadmin" && <NotificationBell />}
              <ThemeToggle />
