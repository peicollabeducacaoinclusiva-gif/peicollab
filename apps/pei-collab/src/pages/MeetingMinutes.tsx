...
import PEIEvaluationInMeeting from "@/components/coordinator/PEIEvaluationInMeeting";
import { 
  Save, 
  CheckCircle, 
  Calendar, 
...
export default function MeetingMinutes() {
  ...
  const [peis, setPeis] = useState<PEIInMeeting[]>([]);
  ...
  const loadPEIs = async () => {
    ...
    try {
      // Buscar PEIs vinculados à reunião
      const { data: meetingPEIs, error } = await supabase
      ...
      if (error) {
        console.error('Erro ao carregar PEIs da reunião:', error);
        return;
      }

      if (meetingPEIs && meetingPEIs.length > 0) {
        const formattedPEIs: PEIInMeeting[] = meetingPEIs.map((mp: any) => ({
          id: mp.pei_id,
          student_name: mp.peis.student?.name || 'Aluno Desconhecido',
          notes: mp.notes || '',
          progress: '',
        }));
        setPeis(formattedPEIs);
        console.log('✅ PEIs carregados:', formattedPEIs.length);
      }
    } catch (error: any) {
  ...
  const savePEIEvaluations = async () => {
    if (!meetingId) return;
    ...
    try {
      ...
      // Salvar avaliações dos PEIs que têm progresso ou notas
      const evaluationsToSave = peis.filter(pei => pei.progress || pei.notes);
      
      for (const pei of evaluationsToSave) {
        if (!pei.progress && !pei.notes) continue;

        // Criar parecer em pei_reviews
        const { error } = await supabase
          .from('pei_reviews')
          .insert({
            pei_id: pei.id,
            reviewer_id: user.id,
            reviewer_role: userRoleData?.role || 'coordinator',
            notes: pei.notes || `Avaliação registrada em reunião: ${pei.progress}`,
            review_date: new Date().toISOString(),
            meeting_id: meetingId,
            evaluation_data: {
              overall_progress: pei.progress,
            },
          });

        if (error) {
          console.error('Erro ao salvar avaliação do PEI:', error);
        }
      }

      console.log(`✅ ${evaluationsToSave.length} avaliações de PEIs salvas`);
    } catch (error: any) {
      console.error('Erro ao salvar avaliações de PEIs:', error);
      // Não throw error aqui para não bloquear salvamento da ata
    }
  };
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      // Salvar avaliações dos PEIs
      await savePEIEvaluations();

      toast({
        title: "Rascunho salvo",
        description: "As alterações e avaliações foram salvas.",
      });
    } catch (error: any) {
      toast({
  ...
  const completeMeeting = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "A ata e as avaliações dos PEIs foram registradas com sucesso!",
      });
