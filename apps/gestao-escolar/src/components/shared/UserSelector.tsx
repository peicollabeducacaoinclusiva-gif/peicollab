...
export function UserSelector({
  ...
  roleFilter,
  schoolFilter,
  placeholder: _placeholder = 'Selecione um usuário',
  label = 'Usuário',
  required = false
}: UserSelectorProps) {
    ...
    try {
      ...
      if (roleFilter && roleFilter.length > 0) {
        filteredData = filteredData.filter(user => {
          ...
          if (!userRoles || Array.isArray(userRoles) === false) return false;
          return (userRoles as any[]).some((r: any) => roleFilter.includes(r.role));
        });
      }
