...
export function usePEIVersioning(peiId: string) {
  ...
  const loadVersions = async () => {
    try {
      ...
      const userIds = [
        ...new Set([
          ...(historyData?.map((v: any) => v.changed_by) || []),
          user?.id
        ])
      ];
      ...
      const processedHistory: PEIVersion[] = historyData?.map((version: any) => ({
        ...version,
      ...
      let processedActive: PEIVersion | null = null;
      if (activeData && typeof activeData === 'object' && 'id' in activeData && !('message' in activeData)) {
        const active = activeData as any;
        processedActive = {
          id: String(active.id),
          pei_id: peiId,
          version_number: Number(active.version_number) || 1,
          status: String(active.status || 'draft'),
          changed_by: userMap.get(user?.id || '') || 'Usuário atual',
          changed_at: new Date().toISOString(),
          change_type: 'current',
          change_summary: 'Versão atual',
          diagnosis_data: active.diagnosis_data || {},
          planning_data: active.planning_data || {},
          evaluation_data: active.evaluation_data || {},
    ...
    try {
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          status: (version.status || 'draft') as any,
          diagnosis_data: version.diagnosis_data,
          planning_data: version.planning_data,
          evaluation_data: version.evaluation_data,
          updated_at: new Date().toISOString()
