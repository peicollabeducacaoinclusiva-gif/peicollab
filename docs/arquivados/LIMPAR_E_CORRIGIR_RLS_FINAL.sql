-- ============================================================================
-- LIMPEZA E CORREÇÃO TOTAL: RLS students e peis
-- ============================================================================
-- Remove TODAS as policies possíveis e recria do zero
-- Execute ESTE SCRIPT ÚNICO no Supabase Dashboard
-- ============================================================================
...
-- Remover todas as variações de policies que podem existir
...
DROP POLICY IF EXISTS "all_view_students" ON "public"."students";

ALTER TABLE "public"."students" ENABLE ROW LEVEL SECURITY;

-- CRIAR POLICIES SIMPLIFICADAS
CREATE POLICY "coord_manage_students"
    ON "public"."students"
    FOR ALL
    USING (
        EXISTS (
...
CREATE POLICY "teacher_view_students"
    ON "public"."students"
    FOR SELECT
    USING (
        EXISTS (
...
CREATE POLICY "family_view_students"
    ON "public"."students"
    FOR SELECT
    USING (
        EXISTS (
...
-- Remover todas as variações de policies
...
DROP POLICY IF EXISTS "admins_manage_peis" ON "public"."peis";
...
DROP POLICY IF EXISTS "teacher_view_assigned_peis" ON "public"."peis";
...
DROP POLICY IF EXISTS "support_professional_view_peis" ON "public"."peis";
DROP POLICY IF EXISTS "pa_view_peis" ON "public"."peis";
...
-- CRIAR POLICIES SIMPLIFICADAS
CREATE POLICY "coord_manage_peis"
    ON "public"."peis"
    FOR ALL
    USING (
        EXISTS (
...
CREATE POLICY "admin_manage_peis"
    ON "public"."peis"
    FOR ALL
    USING (
        EXISTS (
...
CREATE POLICY "teacher_manage_own_peis"
    ON "public"."peis"
    FOR ALL
    USING (created_by = auth.uid());

CREATE POLICY "teacher_view_assigned_peis"
    ON "public"."peis"
    FOR SELECT
    USING (
        EXISTS (
...
CREATE POLICY "family_view_approved"
    ON "public"."peis"
    FOR SELECT
    USING (
        status = 'approved'
