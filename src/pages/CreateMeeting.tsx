  ...
  const loadPEIs = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        console.log('✅ PEIs carregados:', data?.length);
        setAvailablePEIs(data || []);
      }
    } catch (error: any) {
          ...
          <CardContent>
            {availableTeachers.length === 0 ? (
              <div className="text-center py-4 text-muted-foreground">
                <p className="text-sm">Nenhum professor disponível</p>
                <p className="text-xs mt-2">
                  Certifique-se que existem usuários com roles de professor no sistema
                </p>
              </div>
            ) : (
              <>
