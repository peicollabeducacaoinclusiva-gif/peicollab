...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'pei_comments' AND column_name = 'student_id'
  ) THEN
    ALTER TABLE pei_comments ADD COLUMN student_id UUID REFERENCES students(id) ON DELETE CASCADE;
  END IF;
END $$;

-- Populate student_id from peis relationship if null
UPDATE pei_comments 
SET student_id = (
...
-- Make student_id NOT NULL if not already
...
DROP POLICY IF EXISTS "Coordinators can manage student access" ON student_access;
DROP POLICY IF EXISTS "Users can view own student access" ON student_access;
...
DROP POLICY IF EXISTS "Users can view comments on accessible students" ON pei_comments;
DROP POLICY IF EXISTS "Users can insert comments on accessible students" ON pei_comments;
DROP POLICY IF EXISTS "Users can update own comments" ON pei_comments;
DROP POLICY IF EXISTS "Users can delete own comments" ON pei_comments;
...
DROP TRIGGER IF EXISTS maintain_student_access ON peis;
DROP TRIGGER IF EXISTS maintain_coordinator_access ON profiles;
