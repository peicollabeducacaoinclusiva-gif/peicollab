...
-- First, recreate the function with correct column name
...
-- Now drop and recreate the policies
...
DROP POLICY IF EXISTS "Users can view accessible PEIs" ON public.peis;
DROP POLICY IF EXISTS "Teachers can create PEIs" ON public.peis;
DROP POLICY IF EXISTS "Users can update accessible PEIs" ON public.peis;
DROP POLICY IF EXISTS "Users can delete accessible PEIs" ON public.peis;
...
CREATE POLICY "Teachers can create PEIs"
ON public.peis
FOR INSERT
TO authenticated
WITH CHECK (
...
DROP POLICY IF EXISTS "Authorized users can delete students" ON public.students;
