-- Criar tabela de relacionamento entre PEIs e professores (se não existir)
...
-- Habilitar RLS na tabela pei_teachers
ALTER TABLE public.pei_teachers ENABLE ROW LEVEL SECURITY;

-- Políticas RLS para pei_teachers
DROP POLICY IF EXISTS "Teachers can view their PEI assignments" ON public.pei_teachers;
DROP POLICY IF EXISTS "Coordinators can view PEI teachers in their tenants" ON public.pei_teachers;
DROP POLICY IF EXISTS "Coordinators can manage PEI teachers in their tenants" ON public.pei_teachers;
...
DROP POLICY IF EXISTS "users_can_view_accessible_peis" ON public.peis;
DROP POLICY IF EXISTS "users_can_update_accessible_peis" ON public.peis;
DROP POLICY IF EXISTS "users_can_delete_accessible_peis" ON public.peis;
...
DROP POLICY IF EXISTS "users_view_same_tenant" ON public.profiles;
...
DROP TRIGGER IF EXISTS sync_pei_teacher_on_insert_update ON public.peis;
