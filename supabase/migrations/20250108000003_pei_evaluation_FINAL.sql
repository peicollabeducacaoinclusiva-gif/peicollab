-- ============================================================================
-- MIGRAÇÃO FINAL CORRIGIDA: Avaliações Cíclicas do PEI
-- ============================================================================
-- Sistema completo de avaliações por ciclo (I, II, III)
-- Data: 2025-01-08
-- Versão: FINAL - Corrigido erro do CHECK constraint
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS "public"."pei_evaluations" (
    ...
    -- Status (sem CHECK inline, será adicionado depois)
...
-- Adicionar constraint DEPOIS da tabela ser criada
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'pei_evaluations_status_check'
    ) THEN
        ALTER TABLE "public"."pei_evaluations"
        ADD CONSTRAINT "pei_evaluations_status_check"
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'unique_pei_cycle_year'
    ) THEN
        ALTER TABLE "public"."pei_evaluations"
        ADD CONSTRAINT "unique_pei_cycle_year"
        UNIQUE ("pei_id", "cycle_number", "academic_year");
    END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS "idx_pei_evaluations_year" ON "public"."pei_evaluations"("academic_year");
CREATE INDEX IF NOT EXISTS "idx_pei_evaluations_status" ON "public"."pei_evaluations"("status");
...
-- Remover policies existentes primeiro
...
BEGIN
    ...
    SELECT
        ...
        AVG(
            (evaluation_data->>'goals_achieved_percentage')::numeric
...
-- ============================================================================
-- DADOS INICIAIS: Cronograma de Avaliações
-- ============================================================================

-- Inserir cronograma padrão para 2025
...
COMMENT ON COLUMN "public"."pei_evaluations"."cycle_number" IS 'Número do ciclo (1, 2 ou 3)';
COMMENT ON COLUMN "public"."pei_evaluations"."evaluation_data" IS 'Dados estruturados da avaliação em JSON';
COMMENT ON COLUMN "public"."pei_evaluations"."goals_achieved" IS 'Lista de objetivos alcançados';
COMMENT ON COLUMN "public"."evaluation_schedules"."auto_schedule" IS 'Se true, cria automaticamente avaliações para novos PEIs';

-- ============================================================================
-- CONFIRMAÇÃO
-- ============================================================================

SELECT 'Migração 3 aplicada com sucesso! (FINAL - Corrigida)' AS status;
